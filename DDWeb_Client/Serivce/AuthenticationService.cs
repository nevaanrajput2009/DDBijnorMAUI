...
namespace DDWeb_Client.Serivce
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<SignInResponseDTO> Login(SignInRequestDTO signInRequest)
        {
            ...
            var response = await _client.PostAsync($"{IISHelper.AppentUrl}/api/account/signin", bodyContent);
        ...
        public async Task<SignUpResponseDTO> RegisterUser(SignUpRequestDTO signUpRequest)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _client.PostAsync($"{IISHelper.AppentUrl}/api/account/signup", bodyContent);
            var contentTemp = await response.Content.ReadAsStringAsync();
            var result = JsonConvert.DeserializeObject<SignUpResponseDTO>(contentTemp);

            var singInRequest = new SignInRequestDTO
            {
                UserName = signUpRequest.Name,
                Password = signUpRequest.Password,
            };           

            await Login(singInRequest);
            if (response.IsSuccessStatusCode)
            {
